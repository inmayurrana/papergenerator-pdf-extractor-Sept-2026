...
class StorageSyncService {
    static getBaseBankDir() {
        const dataDir = path_1.default.resolve(config_1.config.DATA_DIR);
        const bankDir = path_1.default.join(dataDir, "Bank");
        if (!fs_1.default.existsSync(bankDir)) {
            fs_1.default.mkdirSync(bankDir, { recursive: true });
        }
        return bankDir;
    }
    static getQuestionsBankDir() {
        const dir = path_1.default.join(this.getBaseBankDir(), "QuestionsBank");
        if (!fs_1.default.existsSync(dir)) {
            fs_1.default.mkdirSync(dir, { recursive: true });
        }
        return dir;
    }
    static getQPapersDir() {
        const dir = path_1.default.join(this.getBaseBankDir(), "Qpapers");
        if (!fs_1.default.existsSync(dir)) {
            fs_1.default.mkdirSync(dir, { recursive: true });
        }
        return dir;
     ...
     */
    static async resolveFolderHierarchy(folderId) {
        if (!folderId) {
            return { className: "General", subjectName: "General" };
        }
        try {
            let f = await prisma_1.prisma.folder.findUnique({ where: { id: folderId } });
            if (!f)
                return { className: "General", subjectName: "General" };
            ...
            while (f) {
                ...
                if (f.parentId) {
                    f = await prisma_1.prisma.folder.findUnique({ where: { id: f.parentId } });
                }
                else {
                    break;
                }
    ...
    static async syncAllQuestionsToDisk() {
        ...
        for (const groupKey of Object.keys(grouped)) {
            ...
            const targetDir = path_1.default.join(qBankBaseDir, className, subjectName);
            ...
            fs_1.default.writeFileSync(path_1.default.join(targetDir, "questions.json"), JSON.stringify(jsonExport, null, 2), "utf8");
            ...
            fs_1.default.writeFileSync(path_1.default.join(targetDir, "questions.csv"), csvContent, "utf8");
        }
        return {
    ...
    static async syncPaperToDisk(paperId) {
        ...
        const targetDir = path_1.default.join(this.getQPapersDir(), className, subjectName);
        if (!fs_1.default.existsSync(targetDir)) {
            fs_1.default.mkdirSync(targetDir, { recursive: true });
        }
        ...
        fs_1.default.writeFileSync(path_1.default.join(targetDir, `${baseFileName}.csv`), csvContent, "utf8");
    ...
    `;
        fs_1.default.writeFileSync(path_1.default.join(targetDir, `${baseFileName}.doc`), wordHtml, "utf8");
    ...
    static async restorePapersFromDiskIfEmpty() {
        try {
            const existingCount = await prisma_1.prisma.questionPaper.count();
            if (existingCount > 0)
                return existingCount;
            const qpapersDir = this.getQPapersDir();
            if (!fs_1.default.existsSync(qpapersDir))
                return 0;
            ...
            for (const filePath of jsonFiles) {
                try {
                    ...
                    const existing = await prisma_1.prisma.questionPaper.findFirst({
                        where: { title, examCode },
                    });
                    if (!existing) {
                        await prisma_1.prisma.questionPaper.create({
                            data: {
                                title,
                                examCode,
