...
router.get("/:id", async (req, res) => {
    try {
        const paper = await prisma_1.prisma.questionPaper.findUnique({
...
router.post("/", async (req, res) => {
    try {
        const { title, examCode, schoolName, schoolLogoUrl, instructions, watermark, maxMarks, durationMinutes, examDate, examTime, canvasLayout, currentMarks, } = req.body;
        if (!title || !examCode) {
            res.status(400).json({ error: "Title and Exam Code are required" });
            return;
        }
        let finalCanvasLayout = canvasLayout || {};
        if (typeof finalCanvasLayout === "string") {
            try {
                finalCanvasLayout = JSON.parse(finalCanvasLayout);
            }
            catch { }
        }
        if (!finalCanvasLayout.settings)
            finalCanvasLayout.settings = {};
        if (req.body.className)
            finalCanvasLayout.settings.className = req.body.className;
        if (req.body.subjectName)
            finalCanvasLayout.settings.subjectName = req.body.subjectName;
        const paper = await prisma_1.prisma.questionPaper.create({
            data: {
                title,
                examCode,
        ...
        await (0, audit_1.logAuditAction)(req.user.id, "CREATE_PAPER", "PAPER", paper.id, {
            title: paper.title,
...
router.put("/:id", async (req, res) => {
    try {
        ...
        if (body.examCode !== undefined)
            data.examCode = body.examCode;
        if (body.schoolName !== undefined)
            data.schoolName = body.schoolName;
        if (body.schoolLogoUrl !== undefined)
            data.schoolLogoUrl = body.schoolLogoUrl;
        if (body.instructions !== undefined)
            data.instructions = body.instructions;
        if (body.watermark !== undefined)
            data.watermark = body.watermark;
        if (body.maxMarks !== undefined)
            data.maxMarks = parseInt(body.maxMarks, 10);
        if (body.currentMarks !== undefined)
            data.currentMarks = parseInt(body.currentMarks, 10);
        if (body.durationMinutes !== undefined)
            data.durationMinutes = parseInt(body.durationMinutes, 10);
        if (body.examDate !== undefined)
            data.examDate = body.examDate;
        if (body.examTime !== undefined)
        ...
        if (body.canvasLayout !== undefined || body.canvasLayoutJson !== undefined || body.className !== undefined || body.subjectName !== undefined) {
            ...
            if (rawLayout) {
                ...
                const existing = await prisma_1.prisma.questionPaper.findUnique({ where: { id }, select: { canvasLayoutJson: true } });
                try {
                    layoutObj = JSON.parse(existing?.canvasLayoutJson || "{}");
                }
                catch { }
            }
            if (!layoutObj.settings)
                layoutObj.settings = {};
            if (body.className)
                layoutObj.settings.className = body.className;
            if (body.subjectName)
...
router.post("/:id/finalize", async (req, res) => {
    try {
        ...
        const versionCount = await prisma_1.prisma.paperSnapshot.count({ where: { paperId: id } });
        ...
        const snapshot = await prisma_1.prisma.paperSnapshot.create({
            data: {
                paperId: id,
                version: newVersion,
        ...
        await storageSync_service_1.StorageSyncService.syncPaperToDisk(id);
        await (0, audit_1.logAuditAction)(req.user.id, "FINALIZE_PAPER", "PAPER_SNAPSHOT", snapshot.id, {
            paperId: id,
            version: newVersion,
            maxMarks: paper.maxMarks,
        });
...
router.post("/:id/clone", async (req, res) => {
    try {
        ...
        const clonedPaper = await prisma_1.prisma.questionPaper.create({
            data: {
        ...
        await storageSync_service_1.StorageSyncService.syncPaperToDisk(clonedPaper.id);
        await (0, audit_1.logAuditAction)(req.user.id, "CLONE_PAPER", "PAPER", clonedPaper.id, {
            sourceId: id,
          ...
          ${(settings.customHeaderFields || []).length > 0 ? `
            <tr>
              <td colspan="3" style="padding-top: 3pt; font-size: 9.5pt;">
                ${(settings.customHeaderFields || []).map((f) => `<strong>${f.label}:</strong> ${f.value}`).join(' &nbsp;|&nbsp; ')}
              </td>
            </tr>
          ` : ''}
        </table>
            ...
            ${(settings.customCandidateFields || []).length > 0 ? `
              <tr>
                <td colspan="2" style="padding-top: 4pt;">
                  ${(settings.customCandidateFields || []).map((cf) => `<strong>${cf.label}:</strong> ${cf.placeholder}`).join(' &nbsp;&nbsp;&nbsp;&nbsp; ')}
                </td>
              </tr>
            ` : ''}
          </table>
...
router.get("/:id/export/pdf", async (req, res) => {
    try {
        ...
        const aiRes = await axios_1.default.post(`${config_1.config.AI_SERVICE_URL}/api/export/paper-pdf`, {
...
router.post("/import/json", async (req, res) => {
    try {
        ...
        const totalMarks = canvasLayout.questions.reduce((sum, q) => sum + (q.marks || 1), 0);
        const paper = await prisma_1.prisma.questionPaper.create({
            data: {
        ...
        await storageSync_service_1.StorageSyncService.syncPaperToDisk(paper.id);
        await (0, audit_1.logAuditAction)(req.user.id, "IMPORT_PAPER_JSON", "PAPER", paper.id, {
            title: paper.title,
