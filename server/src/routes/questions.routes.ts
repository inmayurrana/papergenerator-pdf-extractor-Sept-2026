...
const imageStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    const dest = path.resolve(config.DATA_DIR, "diagrams");
...
router.post("/questions/upload-image", uploadImage.single("image"), async (req: AuthRequest, res: Response) => {
  try {
    if (!req.file) {
...
router.post("/folders", async (req: AuthRequest, res: Response) => {
  try {
    ...
    await logAuditAction(req.user!.id, "CREATE_FOLDER", "FOLDER", folder.id, { name, type, parentId });
...
router.delete("/folders/:id", async (req: AuthRequest, res: Response) => {
  try {
    ...
    await logAuditAction(req.user!.id, "DELETE_FOLDER", "FOLDER", req.params.id);
...
router.get("/questions/check-duplicates", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const getOptionsNorm = (q: any): string => {
      try {
        const opts = typeof q.optionsJson === "string" ? JSON.parse(q.optionsJson) : q.options || [];
        return opts
          .map((o: any) => normalize(o.text || "") + (o.imageUrl ? `@img:${o.imageUrl}` : ""))
          .filter(Boolean)
          .join("###");
      } catch {
        return "";
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (processedIds.has(qA.id)) continue;

      const normA = normalize(qA.questionText);
      const optsA = getOptionsNorm(qA);
      const diagsA = getDiagramsNorm(qA);

      const currentDups: Array<{ question: any; similarity: number; reason: string }> = [];

      for (let j = i + 1; j < questions.length; j++) {
        const qB = questions[j];
        if (processedIds.has(qB.id)) continue;
...
router.post("/questions/delete-all-duplicates", async (req: AuthRequest, res: Response) => {
  try {
    const questions = await prisma.question.findMany({
...
router.post("/questions", async (req: AuthRequest, res: Response) => {
  try {
    ...
    await logAuditAction(req.user!.id, "CREATE_QUESTION", "QUESTION", q.id, {
...
router.post("/questions/batch", async (req: AuthRequest, res: Response) => {
  try {
    ...
    await logAuditAction(req.user!.id, "BATCH_IMPORT_QUESTIONS", "QUESTION", "BATCH", {
...
router.put("/questions/:id", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (body.marks !== undefined) data.marks = parseInt(body.marks, 10) || 1;
    ...
    if (body.isRestricted !== undefined) data.isRestricted = Boolean(body.isRestricted);
    if (body.isApproved !== undefined) data.isApproved = Boolean(body.isApproved);

    if (body.subquestions !== undefined) data.subquestionsJson = JSON.stringify(body.subquestions || []);
    if (body.options !== undefined) data.optionsJson = JSON.stringify(body.options || []);
...
router.post("/questions/check-duplicate", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const inputWords = new Set<string>(cleanInput.split(/\s+/).filter(Boolean));
    ...
    for (const q of allQuestions) {
      const qWords = new Set<string>(q.questionText.toLowerCase().replace(/[^a-z0-9]/g, " ").trim().split(/\s+/).filter(Boolean));
      // Jaccard similarity
      const intersection = new Set([...inputWords].filter((x: string) => qWords.has(x)));
      const union = new Set<string>([...inputWords, ...qWords]);
...
router.get("/questions/export/pdf", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const aiRes = await axios.post(
      `${config.AI_SERVICE_URL}/api/export/questions-pdf`,
      {
        questions: formatted,
...
router.post("/questions/import/json", async (req: AuthRequest, res: Response) => {
  try {
    ...
    await logAuditAction(req.user!.id, "IMPORT_QUESTIONS_JSON", "QUESTION", undefined, {
      count: createdQuestions.length,
      folderId,
    });
...
router.post("/questions/import/file", uploadDoc.single("file"), async (req: AuthRequest, res: Response) => {
  try {
    if (!req.file) {
    ...
    const aiRes = await axios.post(`${config.AI_SERVICE_URL}/api/import/parse-questions-file`, params, {
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
    });

    const parsedQuestions: any[] = aiRes.data.questions || [];
    ...
    await logAuditAction(req.user!.id, "IMPORT_QUESTIONS_FILE", "QUESTION", undefined, {
